...
class InfosCollection {
	...
	function getAllInfosByModelID($model_id)
	{
		...
		$sql = "SELECT * FROM glpi_plugin_data_injection_infos WHERE model_id=".$model_id." ORDER BY type ASC";
		$result = $DB->query($sql);
...
class InjectionInfos {
	private $type;
	private $field;
	private $value;
	
	function InjectionInfos($type,$field,$value)
	{
		$this->type = $type;
		$this->field = $field;
		$this->value = $value;
