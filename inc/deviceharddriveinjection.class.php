...
class PluginDatainjectionDeviceHardDriveInjection extends DeviceHardDrive
   ...
   function processAfterInsertOrUpdate($values, $add = true, $rights = array()) {
      if (isset($values['Computer']['id'])) {
         ...
         if (!countElementsInTable($computer_device->getTable(), 
                                   "`deviceharddrives_id`='".$values[get_parent_class($this)]['id']."' 
                                       AND `computers_id`='".$values['Computer']['id']."'")) {
            $tmp['deviceharddrives_id'] = $values[get_parent_class($this)]['id'];
