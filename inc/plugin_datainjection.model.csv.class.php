...
class DataInjectionModelCSV extends DataInjectionModel {
	...
	{
		global $DB;
		$sql = "SELECT * FROM glpi_plugin_datainjection_models_csv WHERE model_id = ".$this->fields["ID"];
		$result = $DB->query($sql);
	...
	function updateSpecificFields()
	{
		...
		$sql = "UPDATE glpi_plugin_datainjection_models_csv SET delimiter='".$this->specific_fields["delimiter"]
	...
	{
		global $DB;
		$sql = "INSERT INTO glpi_plugin_datainjection_models_csv (`delimiter`,`header_present`,`model_id`) VALUES ('".$this->specific_fields["delimiter"]
	...
	{
		global $DB;
		$sql = "DELETE FROM glpi_plugin_datainjection_models_csv WHERE model_id=".$this->fields["ID"];
		$DB->query($sql);
	}
