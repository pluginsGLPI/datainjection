...
$NEEDED_ITEMS=array("ocsng","user","contact","enterprise","contract","networking","group","monitor","phone","infocom","printer","profile","entity",
	...
	"rule.dictionnary.software","device","document","knowbase");
include (GLPI_ROOT."/inc/includes.php");

plugin_datainjection_loadHook("variables");

commonHeader($LANG["datainjection"]["config"][1],$_SERVER["PHP_SELF"],"plugins","datainjection");
...
foreach($_POST as $key => $val)
	if($_SESSION["plugin_datainjection"]["load"]==$key)
		$load=0;
/**************************************************************/
...
if($load)
	...
	if(isset($_POST["next_choiceStep"]))
		...
		switch($_POST["choice"])
			...
			break;
			case 3:
				$_SESSION["plugin_datainjection"]["nbonglet"] = 2;
				$_SESSION["plugin_datainjection"]["choice"] = 3;
	...
	/***************************Save Step**************************/
		...
		if($_SESSION["plugin_datainjection"]["choice"]==1)
			...
			$_SESSION["plugin_datainjection"]["load"] = "yes2_saveStep";
			}
		else
			$_SESSION["plugin_datainjection"]["step"] = 2;
		$_SESSION["plugin_datainjection"]["choice"] = 4;
		$_SESSION["plugin_datainjection"]["nbonglet"] = 5;
		}
...
/********************Initialization****************************/
if($_SESSION["plugin_datainjection"]["step"]==1)
	initSession();
/**************************************************************/
...
/**********************Hide onglet*****************************/
echo "<script type='text/javascript'>deleteOnglet(".$_SESSION["plugin_datainjection"]["nbonglet"].")</script>";
/**************************************************************/


/*********************Select onglet****************************/
echo "<script type='text/javascript'>change_color_step('step".$_SESSION["plugin_datainjection"]["step"]."')</script>";
/**************************************************************/
...
if($_SESSION["plugin_datainjection"]["step"]==1)
	...
	switch($_SESSION["plugin_datainjection"]["choice"])
		{
		case 1:
